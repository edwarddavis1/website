// Modern script for Ed Davis's website
  ...
  try {
    ...
    setTimeout(() => {
      if (iframe.contentWindow.document.body) {
        ...
        if (height > minHeight) {
          const container = iframe.closest('.iframe-container, .iframe-fullwidth');
          if (container) {
            if (container.classList.contains('iframe-container')) {
              container.style.height = `${height + 40}px`; // Add padding
              container.style.paddingBottom = '0';
              console.log(`Resized iframe container to ${height + 40}px for ${iframe.src}`);
            } else {
              container.style.height = `${height + 40}px`;
              console.log(`Resized iframe-fullwidth to ${height + 40}px for ${iframe.src}`);
            }
          }
        }
      }
    ...
    console.log(`Cross-origin iframe detected for ${iframe.src}, using default sizing`);
  }
}

// Handle all iframes on the page
function initializeIframes() {
  const iframeContainers = document.querySelectorAll('[data-iframe-resize="true"]');
  console.log(`Found ${iframeContainers.length} iframe containers to initialize`);
  
  iframeContainers.forEach(container => {
    const iframe = container.querySelector('iframe');
    if (iframe) {
      // Set a minimum height for the container initially
      const defaultHeight = container.classList.contains('iframe-fullwidth') ? '600px' : '400px';
      container.style.minHeight = defaultHeight;
      console.log(`Setting minimum height ${defaultHeight} for iframe: ${iframe.src}`);
      ...
      window.addEventListener('resize', () => {
        ...
        if (window.innerWidth < 768) {
          if (container.classList.contains('iframe-container')) {
            ...
            container.style.paddingBottom = container.classList.contains('dynamic') ? '0' : '56.25%';
          }
        } else {
...
const sectionObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      ...
      const animElements = entry.target.querySelectorAll('.anim-fade-in, .anim-fade-up, .anim-fade-right');
